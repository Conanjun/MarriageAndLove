...
<div class="font">
	...
	<div class="div2"><img src="<%=ROOTPATH %>/static/image/9.png" class="img2"/>&nbsp&nbsp好友列表</div><img src="<%=ROOTPATH %>/static/image/11.png"><br />
		...
		<% if(isInOnlineUserReal(friendlist[i])){ %>
				    ...
				    离线
        &nbsp;&nbsp;&nbsp;
		<a href="show_friendinfo.aspx?friendname_dtl=<%=friendlist[i]%>" target="_parent">查看好友<img src="<%=ROOTPATH %>/static/image/14.png" class="img3"/></a>&nbsp;&nbsp;
					 <%} %>
                        
					<a href="friend_msg.aspx?friendname_msg=<%=friendlist[i]%>" target="_parent">发送消息<img src="<%=ROOTPATH %>/static/image/13.png" class="img3"/></a>&nbsp;&nbsp;
					<a href="../chat/chatroom.aspx?launch_username=<%=Session["username"]%>&invited_username=<%=friendlist[i]%>">发起实时聊天<img src="<%=ROOTPATH %>/static/image/zaixianxiaoxi.png" class="img3"/></a>&nbsp;&nbsp;
					 <%--实时聊天功能暂时共用一个聊天室，想到别的解决办法再改--%>

				    <a href="friend_del.aspx?friendname_del=<%=friendlist[i]%>" target="_parent">删除好友<img src="<%=ROOTPATH %>/static/image/15.png" class="img3"/></a>
...
&nbsp;&nbsp;&nbsp;
			...
			<asp:Button ID="search" class="btn btn-6 btn-6a" runat="server" Text="查找" OnClick="search_Click" BorderStyle="NotSet" />
		</div>
